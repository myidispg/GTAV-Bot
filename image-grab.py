...
def draw_lanes(img, lines, color=[0, 255, 255], thickness=3):
    ...
    try:
        ...
        # finds the maximum y value for lane marker
        ...
        for i in lines:
            for ii in i:
                ys += [ii[1], ii[3]]
        # Here we find the min value of y because min y will be the highest point on the screen.
        # draw till the max y.
        min_y = min(ys)
        max_y = 600
        new_lines = []
        ...
        for idx, i in enumerate(lines):
            for xyxy in i:
                ...
                x_cords = (xyxy[0], xyxy[2])
                y_cords = (xyxy[1], xyxy[3])
                a = vstack([x_cords, ones(len(x_cords))]).T
                m, b = lstsq(a, y_cords)[0]
                
                # calculate new and improved xs
                x1 = (min_y - b) / m
                x2 = (max_y - b) / m
